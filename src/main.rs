...
async fn main() -> Result<(), Error> {
    ...
    auth::initialize_keys().await.unwrap_or_else(|e| {
        error!("Error creating private key '{}'\n{e:?}\nExiting Vaultwarden!", CONFIG.private_ed25519_key());
        exit(1);
    });
    check_web_vault();
...
async fn perform_healthcheck(custom_endpoint: Option<String>) -> Result<(), Error> {
    ...
    let healthcheck_url = if let Some(endpoint) = custom_endpoint {
        ...
        let rocket_port = std::env::var("ROCKET_PORT").unwrap_or_else(|_| "80".to_owned());
        format!("http://localhost:{rocket_port}/alive")
    };
    ...
    match client.get(&healthcheck_url).send().await {
        ...
        Err(e) => {
            error!("Healthcheck to {healthcheck_url} failed: {e}");
            Err(e.into())
        }
    }
}
