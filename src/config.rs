...
macro_rules! make_config {
    ...
    ( @supportstr $name:ident, $value:expr_2021, $ty:ty, option ) => { serde_json::to_value(&$value).unwrap() }; // Optional other or string, we convert to json
    ( @supportstr $name:ident, $value:expr_2021, String, $none_action:ident ) => { $value.as_str().into() }; // Required string value, we convert to json
    ( @supportstr $name:ident, $value:expr_2021, $ty:ty, $none_action:ident ) => { ($value).into() }; // Required other value, we return as is or convert to json
