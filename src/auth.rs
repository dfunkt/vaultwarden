...
pub async fn initialize_keys() -> Result<(), Error> {
    ...
    let ed25519_key_filename = crate::storage::file_name(&CONFIG.private_ed25519_key())
        .ok_or_else(|| IoError::other("Private Ed25519 key path missing filename"))?;

    let operator = CONFIG.opendal_operator_for_path_type(&PathType::Ed25519Key).map_err(IoError::other)?;
